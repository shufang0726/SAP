...
namespace Automation_Android.Utility
{
    ...
    {
        static OpenQA.Selenium.Remote.RemoteWebDriver theDriver;
        ...
        #region Session
        public static void InitialAndroid(string browserName, string version, string deviceName)
        {
        ...
        public static void openInstalApp(string version, string deviceName, string uri, string appPackage, string appActivity)
        {
            InitialAndroid("", version, deviceName);
            capabilities.SetCapability("appPackage", appPackage);
            capabilities.SetCapability("appActivity", appActivity);
            theDriver = new RemoteWebDriver(new Uri(uri), capabilities);
            myWait = new WebDriverWait(theDriver, TimeSpan.FromSeconds(DEFAULT_TIMEOUT));
            myJsExecutor = (IJavaScriptExecutor)theDriver;
            myActions = new Actions(theDriver);
            theDriver.Manage().Window.Maximize();

        }
        public static void openUNInstalApp(string version, string deviceName, string uri, string appPackagePath)
        {
            InitialAndroid("", version, deviceName);
            capabilities.SetCapability("appActivity", appPackagePath);
            theDriver = new RemoteWebDriver(new Uri(uri), capabilities);
            myWait = new WebDriverWait(theDriver, TimeSpan.FromSeconds(DEFAULT_TIMEOUT));
            myJsExecutor = (IJavaScriptExecutor)theDriver;
            myActions = new Actions(theDriver);
            theDriver.Manage().Window.Maximize();
            rootWindowHandle = theDriver.CurrentWindowHandle;
        ...
        {
            string langRFCName = WebDriverUtility.getValueRegEdit();
        ...
        private static IWebElement getElement(ISearchContext context, By by)
        {
            ...
            else if (theDriver != null)
            {
                ...
                element = myWait.Until(d =>
                {
                    ...
                    try
                    {
                        result = theDriver.FindElement(by);
                    }
                    catch (NoSuchElementException)
                    {
                    }
        ...
        private static ReadOnlyCollection<IWebElement> getElements(ISearchContext context, By by)
        {
            ...
            {
                return theDriver.FindElements(by);
            }
            else
            {
                return null;
        ...
        public static void OpenUrl(string url)
        {
            ...
            theDriver.Navigate().GoToUrl(url);
        }
        ...
        {
            return theDriver.FindElements(by).Count != 0;
        }
        public static bool IsExist(By by)
        {
            ...
            try
            {
                WebDriverWait innerWait = new WebDriverWait(theDriver, TimeSpan.FromSeconds(3));
                result = innerWait.Until(d => { return theDriver.FindElements(by).Count > 0; });
                if (result)
                {
                    if (theDriver.FindElement(by).Displayed)
                        result = true;
                    else
                        result = false;
                }
        ...
        public static void TryToClsoeAlertByAccept()
        {
            ...
            {
                PauseSecond();
                IAlert alert = theDriver.SwitchTo().Alert();
                alert.Accept();
        ...
        public static void clickOMS(string parentXPath, string ChildXPth)
        {
            ...
            IWebElement parentElement = theDriver.FindElement(By.XPath(parentXPath));
            if (parentElement.Displayed)
            {
                action.MoveToElement(parentElement).Build().Perform();

            }
            IJavaScriptExecutor js = (IJavaScriptExecutor)theDriver;
            if (Exist(By.XPath(ChildXPth)))
            {
                IWebElement childElement = theDriver.FindElement(By.XPath(ChildXPth));
        ...
        public static void WaitForElementLoaded(string cssSelector)
        {
            ...
            myWait.Until(p =>
            {
                ...
                try
                {
                    result = theDriver.FindElements(By.CssSelector(cssSelector)).Count > 0;
                }
                catch (NoSuchElementException)
                { }
            ...
            myWait.Until(p =>
            {
                ...
                try
                {
                    result = theDriver.FindElement(By.CssSelector(cssSelector)).Displayed;
                }
                catch (NoSuchElementException)
                { }
        ...
        {
            myWait.Until(q => { return theDriver.WindowHandles.Count > 1; });

            if (theDriver.CurrentWindowHandle == rootWindowHandle)
            {
                foreach (string handle in theDriver.WindowHandles)
                {
                    if (!handle.Equals(rootWindowHandle))
                    {
                        theDriver.SwitchTo().Window(handle);
                        myWait.Until(q =>
                        {
        ...
        {
            theDriver.SwitchTo().Window(rootWindowHandle);
        }
